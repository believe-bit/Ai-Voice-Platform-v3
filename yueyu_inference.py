...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output_path', '--output', type=str, default="output/vits", help='输出文件路径')  # 已添加 --output 别名
    ...
    parser.add_argument('-s', '--spk', type=str, help='合成目标说话人名称或ID')
    # 可选参数
    ...
    if language is not None:
        # 如果是粤语，转换汉字为Jyutping
        if language in ["Cantonese", "yueyu"]:  # 支持 yueyu 和 Cantonese
            text = convert_to_jyutping(text)
        text = language_marks[language] + text + language_marks[language]
        
        # 处理 spk（说话人）
        if spk.isdigit():  # 如果 spk 是数字字符串
            spk_id = int(spk)
            # 查找 hps.speakers 中对应的说话人名称
            for speaker_name, sid in speaker_ids.items():
                if sid == spk_id:
                    spk = speaker_name
                    break
            else:
                raise ValueError(f"No speaker found for ID {spk} in hps.speakers: {speaker_ids}")
