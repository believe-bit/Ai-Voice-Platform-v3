...
def run(rank, n_gpus, hps):
  ...
  if hps.cont:
      try:
          ...
          print(f"Loaded checkpoint at epoch {epoch_str}, global_step {global_step}")
      except Exception as e:
          print(f"Failed to find latest checkpoint, training from scratch: {str(e)}")
          epoch_str = 1
          global_step = 0
  else:
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  net_g.train()
  net_d.train()
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(tqdm(train_loader, file=sys.stdout)):
      ...
      if global_step % hps.train.eval_interval == 0:
        ...
        if hps.preserved > 0:
          ...
          if os.path.exists(old_g):
            print(f"Removed old checkpoint: {old_g}")
            os.remove(old_g)
          if os.path.exists(old_d):
            print(f"Removed old checkpoint: {old_d}")
            os.remove(old_d)
          if os.path.exists("/content/drive/MyDrive/"):
              utils.save_checkpoint(net_g, None, hps.train.learning_rate, epoch,
