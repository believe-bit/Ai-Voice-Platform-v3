from flask import Flask, request, jsonify, Response, send_from_directory,WebSocket
...
from reportlab.pdfbase import pdfmetrics  # 修复：添加导入
from reportlab.pdfbase.ttfonts import TTFont  # 修复：添加导入
import logging
...
def get_db():
    ...
    conn.row_factory = sqlite3.Row  # 确保返回字典
    try:
        yield conn
    finally:
        conn.close()
    ...
    with get_db() as conn:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS projects
                     (id INTEGER PRIMARY KEY AUTOINCREMENT,
                      ...
                      guide_path TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS distributed_projects
                     (project_id INTEGER,
                      student_ip TEXT,
                      PRIMARY KEY (project_id, student_ip))''')
        c.execute('''CREATE TABLE IF NOT EXISTS student_ips
                     (ip TEXT PRIMARY KEY)''')
        c.execute("PRAGMA table_info(projects)")
        cols = [row[1] for row in c.fetchall()]
        if 'guide_steps' not in cols:
            c.execute('ALTER TABLE projects ADD COLUMN guide_steps TEXT')
        conn.commit()
...
def get_user():
    ...
    try:
        with open(USERS_FILE, 'r', encoding='utf-8') as f:
            users = json.load(f)
        user = next((u for u in users if u['ip'] == client_ip), {'role': 'student'})
...
def get_projects():
    ...
    with get_db() as conn:
        c = conn.cursor()
        if role == 'admin':
            c.execute('SELECT id, name, type, level, created_at, guide_path, guide_steps FROM projects')
            projects = c.fetchall()
        else:
            c.execute('SELECT p.id, p.name, p.type, p.level, p.created_at, p.guide_path, p.guide_steps FROM projects p '
                      'JOIN distributed_projects dp ON p.id = dp.project_id WHERE dp.student_ip = ?', (client_ip,))
            projects = c.fetchall()
...
def create_project():
    role = request.headers.get('X-User-Role', 'student')
    if role != 'admin':
        return jsonify({'error': '仅管理员可创建项目'}), 403
    data = request.get_json()
    if not data or not all(key in data for key in ['name', 'type', 'level']):
    ...
    try:
        with get_db() as conn:
            ...
            c.execute('INSERT INTO projects (name, type, level, created_at) VALUES (?, ?, ?, ?)',
                      (data['name'], data['type'], data['level'], created_at))
            conn.commit()
            project_id = c.lastrowid
            c.execute('SELECT id, name, type, level, created_at, guide_path FROM projects WHERE id = ?', (project_id,))
            project = c.fetchone()
...
def update_project(id):
    role = request.headers.get('X-User-Role', 'student')
    if role != 'admin':
        return jsonify({'error': '仅管理员可更新项目'}), 403
    data = request.get_json()
    if not data or not all(key in data for key in ['name', 'type', 'level']):
        return jsonify({'error': '缺少必要字段'}), 400
    try:
        with get_db() as conn:
            ...
            c.execute('UPDATE projects SET name = ?, type = ?, level = ? WHERE id = ?',
                      (data['name'], data['type'], data['level'], id))
            if c.rowcount == 0:
...
def delete_project(id):
    role = request.headers.get('X-User-Role', 'student')
    if role != 'admin':
        return jsonify({'error': '仅管理员可删除项目'}), 403
    try:
        with get_db() as conn:
            c = conn.cursor()
...
def save_guide():
    role = request.headers.get('X-User-Role', 'student')
    if role != 'admin':
        return jsonify({'error': '仅管理员可保存指导书'}), 403
    try:
        data = request.get_json()
        project_id = data.get('project_id')
        steps = data.get('steps')
        if not project_id or not steps:
            return jsonify({'error': '缺少 project_id 或 steps'}), 400
        if not all(isinstance(step, dict) and 'title' in step and 'content' in step for step in steps):
            return jsonify({'error': 'steps 格式错误'}), 400
        filename = f"guide_{project_id}_{int(time.time())}.pdf"
        filepath = os.path.join(GUIDE_DIR, filename)
        try:
            c = canvas.Canvas(filepath, pagesize=A4)
            # 使用 WenQuanYiZenHei 或 NotoSansCJK
            font_name = 'WenQuanYiZenHei' if pdfmetrics.getFont('WenQuanYiZenHei') else 'NotoSansCJK'
            c.setFont(font_name, 12)
            y = 750
            for step in steps:
                c.drawString(100, y, f"步骤: {step['title']}")
                y -= 20
                lines = simpleSplit(step['content'], font_name, 12, 400)
                for line in lines:
            ...
            return jsonify({'error': f'PDF 创建失败: {str(e)}'}), 500
        with get_db() as conn:
            c = conn.cursor()
            c.execute('UPDATE projects SET guide_path = ? WHERE id = ?', (filename, project_id))
            conn.commit()
        print(f"Guide saved: {filepath}")
        return jsonify({'guide_path': filename})
    except Exception as e:
        print(f"Error saving guide: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def distribute_project(id):
    role = request.headers.get('X-User-Role', 'student')
    if role != 'admin':
        return jsonify({'error': '仅管理员可下发项目'}), 403
    data = request.get_json()
    ips = data.get('ips')
    if not ips:
...
def handle_connect():
    ...
    try:
        with open(USERS_FILE, 'r', encoding='utf-8') as f:
            users = json.load(f)
        if not any(u['ip'] == client_ip for u in users):
            users.append({'ip': client_ip, 'role': 'student'})
            with open(USERS_FILE, 'w', encoding='utf-8') as f:
