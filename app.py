...
def save_guide():
    ...
    if not project_id or not steps:
        return jsonify({'error': '缺少必要字段'}), 400
    guide_path = os.path.join(GUIDE_DIR, f'guide_{project_id}.pdf')
    try:
        ...
        for step in steps:
            ...
            y -= 20
            lines = simpleSplit(content, 'SimSun', 12, 400)
            for line in lines:
        ...
        with get_db() as conn:
            c = conn.cursor()
            c.execute('UPDATE projects SET guide_path = ? WHERE id = ?', (guide_path, project_id))
            conn.commit()
        return jsonify({'message': '指导书保存成功', 'guide_path': guide_path})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
